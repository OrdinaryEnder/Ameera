...
class Run(commands.Cog, name='CodeExecution'):
    ...
    async def log_error(self, error, error_source=None):
        is_context = isinstance(error_source, commands.Context)
    ...
    async def get_api_parameters_with_codeblock(self, ctx):
        print(ctx.message.content.count("```"))
    ...
    @commands.command()
    async def availablelang(self, ctx):
     await ctx.send(", ".join(sorted(set(self.languages.values()))))

    @commands.command()
    async def run(self, ctx, *, source=None):
        """ Run codes with Piston API
            See <prefix> availablelang"""
